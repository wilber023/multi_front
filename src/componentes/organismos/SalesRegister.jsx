            ...
            {showSummary && (
                <div className="popup-background">
                    <section className="resumen">
                        <h2>Sales summary</h2>
                        <p><strong>ID User:</strong> {userId}</p>
                        <p><strong>Quantity:</strong> {quantity}</p>
                        {fishDetails.map((fish, index) => (
                            <p key={index}>
                                <strong>Fish {index + 1}:</strong> Weight - {fish.weight}, Type - {fish.type}, Price - {fish.price}
                            </p>
                        ))}
                        <button onClick={handleConfirmSale} style={{ width: '200px' }}>Confirm and Register Sale</button>
                        <button onClick={handleCancel} style={{ backgroundColor: 'red',width: '200px' }}>Cancel</button>
                    </section>
                </div>
            )}
